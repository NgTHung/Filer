...
async fn create_session(core: &FilerCore) -> crate::model::session::SessionId {
...
async fn wait_for_directory_loaded(
    core: &FilerCore,
    expected_session: crate::model::session::SessionId,
) -> (PathBuf, usize) {
    let rx = core.event_receiver();
...
mod navigation_flow_tests {
    ...
    async fn test_navigate_back_then_forward() {
        ...
        // The Navigator emits CurrentNavigateState *before* triggering the scan,
        // so we must not drain the channel before looking for it.
        let rx = core.event_receiver();
        core.send(Command::NavigateBack(session)).unwrap();

        // Wait for the DirectoryLoaded for /alpha AND the CurrentNavigateState
        // with can_forward = true, whichever arrives first.
        let deadline = tokio::time::Instant::now() + TIMEOUT;
    ...
    async fn test_navigate_emits_nav_state_snapshot() {
        ...
        // Also include the DirectoryLoaded we may have drained
    ...
    async fn test_nav_state_can_back_after_second_navigate() {
        ...
        // Start listening BEFORE sending the second Navigate so we don't miss
        // the CurrentNavigateState snapshot emitted synchronously by the Navigator.
        let rx = core.event_receiver();
        ...
        loop {
            ...
            match timeout(Duration::from_millis(100), rx.recv_async()).await {
                ...
                Ok(Ok(Event::DirectoryLoaded { session: s, .. })) if s == session => {
                    // DirectoryLoaded arrived — if we haven't seen can_back yet,
                    // give a bit more time for the snapshot event to arrive.
                }
                _ => {}
            }
        }
