...
- [ ] A typed project-configuration model loads .tasks/config.json and rejects unsupported versions, unknown fields, duplicate entries, invalid names, and conflicting roles with path-aware errors.
- [ ] The configuration exposes domain definitions and prefixes, task-type definitions, and tag policy through one shared library API without defining an implicit default domain.
