...
- [ ] GET /api/projects/{project}/policy returns the project's domains with prefixes, task types, and tag policy with catalog, read-only.
- [ ] GET /api/projects/{project}/tasks with an unknown tag under strict policy returns the structured tag_rejected error; under open policy the filter applies and may return an empty list.
- [ ] A bare parent filter id matching more than one domain is rejected with filer-task's existing ambiguous-reference error and its candidate context.
- [ ] In-process tests cover the policy shape, strict and open tag filtering, and the ambiguous parent rejection, following the existing oneshot test pattern.
