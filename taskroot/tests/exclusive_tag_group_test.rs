...
fn list_and_ready_filters_combine_triage_tags_with_structural_readiness() {
  ...
  "tags": {
    ...
    let listed = Command::new(env!("CARGO_BIN_EXE_taskroot"))
        .args([
            "list",
            "--root",
            root,
    ...
    let ready = Command::new(env!("CARGO_BIN_EXE_taskroot"))
        .args([
            "ready",
            "--root",
            root,
...
fn cli_sets_an_exclusive_tag_group_value() {
  ...
  "tags": {
    ...
    let rejected = Command::new(env!("CARGO_BIN_EXE_taskroot"))
        .args(["tag", "set", "--root"])
        .arg(repo.path())
    ...
    let cleared = Command::new(env!("CARGO_BIN_EXE_taskroot"))
        .args(["tag", "clear", "--root"])
        .arg(repo.path())
