...
fn configured_types_round_trip_and_drive_criteria() {
    ...
    let cli_add = Command::new(env!("CARGO_BIN_EXE_taskroot"))
        .args([
            "add",
            "--domain",
            "backend",
    ...
    let cli_list = Command::new(env!("CARGO_BIN_EXE_taskroot"))
...
fn cli_help_describes_configured_types_tags_and_milestone_roles() {
    let add = Command::new(env!("CARGO_BIN_EXE_taskroot"))
        .args(["add", "--help"])
        .output()
    ...
    let milestone = Command::new(env!("CARGO_BIN_EXE_taskroot"))
        .args(["milestone", "--help"])
        .output()
